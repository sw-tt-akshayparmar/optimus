...
export const BAR_CHART_SPEC = {
  ...
  description:
    "Rotating globe animation depicting earthquake locations. Background - pseudorandomly distributed 'stars'.",
  width: 600,
  height: 600,
  padding: 5,
  autosize: 'pad',
      ...
      on: [
        {
          events: 'timer{100}',
          update:
            'pauseClicked ? globeRotationAngle : (globeRotationAngle<=-180?180:globeRotationAngle-1)',
        },
      ],
    },
  ],
  data: [
    ...
    },
    {
      ...
      url: 'https://vega.github.io/editor/data/earthquakes.json',
      ...
      encode: {
        symbols: {
          update: {
            size: {
              signal:
                "datum.value<1?0:3.14*scale('scaleSize', exp(datum.value))*scale('scaleSize', exp(datum.value))",
            },
